...
describe('/research-review path confinement (#212)', () => {
  it('rejects out-of-workspace absolute paths before any reviewer starts', async () => {
    ...
    // An existing file OUTSIDE the workspace: on Windows `/etc/passwd` does
    // not resolve, and the not-exist short-circuit would mask the confinement
    // error under test.
    const outside = join(mkdtempSync(join(tmpdir(), 'mimir-review-outside-')), 'target.md')
    await writeFile(outside, '# outside\n')

    const execution = await ctx.commands.execute(agent, `/research-review audit ${outside}`, [], new AbortController().signal)
    ...
    expect(execution?.result.text).toContain(outside)
  })

  it.skipIf(!canCreateSymlink())('rejects an in-workspace symlink whose target escapes', async () => {
...
describe('reviewPathsOutsideWorkspace (#212)', () => {
  it('confines in-workspace files and flags everything else', async () => {
    ...
    // The symlink leg needs real symlinks (Windows without Developer Mode
    // cannot create them); the lexical legs above cover it everywhere.
    if (canCreateSymlink()) {
